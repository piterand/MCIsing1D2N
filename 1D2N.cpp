...
srand((unsigned)time(NULL)+rank);
 int CO = Prohod_MC_sampling/CountOut;
    ...
    ofstream outAM("AMagn.dat",ios::out);    
    fstream outAM2("AMagn2.dat",ios::out);         
    ofstream outAM4("AMagn4.dat",ios::out);       //Средняя намагниченность
    ...
    // avgValueMagn.insert(avgValueMagn.end(),temMagn);  //средняя MAGN
    // avgValuePP.insert(avgValuePP.end(),temPP); // записываем в вектор для сохранения
    // avgValuePPF.insert(avgValuePPF.end(),temPPF); // записываем в вектор для сохранения
    // avgValueMagn.insert(avgValueMagn.end(),temMagn); // записываем в вектор для сохранения
    // avgValue.insert(avgValue.end(),Aenergy);
    // avgValue2.insert(avgValue2.end(),Aenergy2);
    // avgValue4.insert(avgValue4.end(),Aenegry4);
    //теплоёмкость
    ...
    if(rank==0) //countOut - количество выводов!!! CO- какое значение из общего количества
    {
        for(int i=0; i<size; ++i)
        {
            ...
            // outAPPF<<(i+1)*((maxtemp-mintemp)/size)<<'\t'<<recvAPPF[i]<<endl;
            // outAPP<<(i+1)*((maxtemp-mintemp)/size)<<'\t'<<recvAPP[i]<<endl;
